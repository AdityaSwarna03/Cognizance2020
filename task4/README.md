...
.Caluclates and autputs the points for each round and each player's total score.
...
Steps of flogarithm:
.First accepts input from players.
.Then it accepts every input for each turns.
.It takes the input from the main.
.The inputs will be processed in the play function.
.After accepting 5 turns from each player then it adds all turns.
.Finally gives player1 is winner if sum of player1 is more than player2 and viceversa.
Image corner:
